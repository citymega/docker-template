...
docker build --build-arg IMAGE_VERSION=${HTTPD_FULL_VERSION} --build-arg \
 BUILD_DATE=$(date -u +'%Y-%m-%dT%H:%M:%SZ') -t citymega/httpd:latest -f "${SCRIPTDIR}/Dockerfile" "${SCRIPTDIR}/.."
...
docker image tag citymega/httpd:latest citymega/httpd:2.4
docker image tag citymega/httpd:latest citymega/httpd:${HTTPD_VERSION}
docker image tag citymega/httpd:latest citymega/httpd:${HTTPD_FULL_VERSION}
docker push citymega/httpd:2.4
docker push citymega/httpd:${HTTPD_VERSION}
docker push citymega/httpd:${HTTPD_FULL_VERSION}
docker push citymega/httpd:latest
